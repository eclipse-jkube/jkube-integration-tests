...
class OpenLibertyK8sITCase extends OpenLiberty {
  ...
  void k8sResource() throws Exception {
    ...
    final File metaInfDirectory = new File(
      String.format("../%s/target/classes/META-INF", PROJECT_OPENLIBERTY));
    assertThat(metaInfDirectory.exists(), equalTo(true));
    assertListResource(new File(metaInfDirectory, "jkube/kubernetes.yml"));
    assertThat(new File(metaInfDirectory, "jkube/kubernetes/openliberty-rest-deployment.yml"), yaml(not(anEmptyMap())));
    assertThat(new File(metaInfDirectory, "jkube/kubernetes/openliberty-rest-service.yml"), yaml(not(anEmptyMap())));
  }
  ...
  void k8sApply() throws Exception {
    ...
    awaitDeployment(this, pod.getMetadata().getNamespace())
      ...
      .assertContainers(hasItems(allOf(
        hasProperty("image", equalTo("integration-tests/openliberty-rest:latest")),
        hasProperty("name", equalTo("openliberty")),
        hasProperty("ports", hasSize(3)),
        hasProperty("ports", hasItems(allOf(
          hasProperty("name", equalTo("glrpc")),
          hasProperty("containerPort", equalTo(9080))
        )))
      )));
  }
